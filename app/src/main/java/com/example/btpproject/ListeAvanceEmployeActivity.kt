...
class ListeAvanceEmployeActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val conn = ListeAvance().execute(id_chantier.toString(), url, db, username, password)
        val list = conn.get()
     ...
     if(list!=null){   for (i in 0..(list!!.size) - 1) {
        ...
        val connEmp = ListeEmploye().execute(url, db, username, password)
    ...
    idE=jsonArray3.getJSONObject(i).getString("id").toInt()
                ...
                if(montant != "" && date != "" && empl != "") {
                    val demandeAvance = AjouterDemandeAvance().execute(
                        ...
                        date,
                        idE.toString(),
                        montant, url, db, username, password
                    )
                    mBuilder.dismiss()
                    val i:Intent=intent
                    finish()
